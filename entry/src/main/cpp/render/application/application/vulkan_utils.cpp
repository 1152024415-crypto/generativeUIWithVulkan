...
// Vulkan dynamic loading is only needed on Windows
// On HarmonyOS, Vulkan is statically linked
...
namespace utils {
    ...
    bool LoadVulkanLibrary()
    {
        ...
        if (!g_libVulkan) {
            ...
            LOGE("Failed to load vulkan library: %s", error ? error : "unknown error");
            return false;
        }

        LOGI("Vulkan library loaded at %p", g_libVulkan);

        // Load vkGetInstanceProcAddr first using dlsym
        void* sym = dlsym(g_libVulkan, "vkGetInstanceProcAddr");
        if (!sym) {
        ...
        // Load vkCreateInstance using vkGetInstanceProcAddr with NULL instance
        // This is the Vulkan-recommended way to get core functions
        ...
        LOGI("All base Vulkan functions loaded successfully");
        return true;
    }
